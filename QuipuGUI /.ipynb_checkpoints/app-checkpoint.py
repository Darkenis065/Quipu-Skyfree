...
if st.session_state.current_page == 'orbital':
    ...
    # Para otras páginas, mostrar contenido normal con diseño espacial
    st.markdown("""
    <style>
    .space-page {{
        margin-top: 80px;
    ...
    if st.session_state.current_page == 'analisis':
        st.markdown('<h1 class="space-title">📊 Cálculo de Datos</h1>', unsafe_allow_html=True)
        st.write("Herramientas avanzadas para análisis de datos astronómicos")
        # Aquí puedes agregar tus componentes de análisis
    ...
    # Botón para volver usando Streamlit nativo
    if st.button("← Volver al Portal Principal", key="back_button"):
        st.session_state.current_page = 'orbital'
        st.experimental_set_query_params()
        st.rerun()
